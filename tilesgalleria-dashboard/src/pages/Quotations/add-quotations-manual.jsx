...
export default function AddQuotationManual() {
  ...
  const handleSaveQuotation = async () => {
    try {
      ...
      const payload = {
        quoNo: `MQUO-${Date.now()}`,
        customer: customerDetails || { name: "", phone: "", address: "" },
        items: itemsData,
      ...
      await axios.post("http://localhost:8080/api/manualquotations/manual", payload, {
        headers: { Authorization: `Bearer ${token}` },
      });

      alert("✅ Manual Quotation saved successfully!");
    } catch (err) {
      console.error("Save manual quotation failed", err.response?.data || err.message);
      alert("❌ Failed to save manual quotation");
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-white rounded-lg shadow">
      <h2 className="text-2xl font-bold text-indigo-700 mb-6">➕ Add Manual Quotation</h2>
